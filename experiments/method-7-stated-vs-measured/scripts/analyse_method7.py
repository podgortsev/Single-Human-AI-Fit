...
the two side by side.
  ...
  STATED    mean self-rating(signal) - mean self-rating(NONE), 0-10 scale.
            NONE is the baseline: how much the model claims context matters when
...
def main():
    ...
    print("=" * 96)
    print("STATED   = mean self-rating(signal) - self-rating(NONE baseline), 0-10.")
    print("           A plain difference of means, NOT a standardised effect size.")
    ...
    print(f"{'model':9}{'signal':8}{'basel.':>7}{'stated dif':>11}{'BH':>9}"
    ...
    print("""
"basel." is the model's mean self-rating for the NONE condition: with nothing
disclosed, how much it still claims something about the user changed its answer.
It is a generic self-attribution baseline, and deliberately not called a floor:
it is not the same construct as method 2's signal-free floor.

Note what the self-report is: the model is asked AFTER it has seen its own
answer and the disclosure, and it is never shown the counterfactual answer
without the disclosure. So this measures post-hoc attribution, not introspective
access to the causal effect.
